...
namespace Omni.Core.Components
{
    ...
    public class NetworkPhysicsIsolate : MonoBehaviour
    {
        ...
        [SerializeField] [GroupNext("Material Settings")]
        ...
        {
            m_ServerIdentities = ServerSide.Identities.Values.Select(x => x.GetComponentsInChildren<Collider>(true))
                .ToList();
            m_ClientIdentities = ClientSide.Identities.Values.Select(x => x.GetComponentsInChildren<Collider>(true))
                .ToList();
