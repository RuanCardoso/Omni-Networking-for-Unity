...
public sealed class NetworkBandwidthDisplay : OmniBehaviour
{
    ...
    [InfoBox(
        "If enabled, displays only the bytes of the Omni payload (excluding transport overhead). " +
        "When disabled, includes both the Omni payload and transport headers, but not lower-level " +
        "network overhead (such as IP/UDP/TCP or Ethernet headers). Actual wire bandwidth may be " +
        "slightly higher than displayed.",
        TriMessageType.Info
    )]
    [OnValueChanged(nameof(UpdateBandwidthPayloadOnly)), ReadOnly]
    private bool m_BandwidthPayloadOnly = true;
    ...
    private void UpdateBandwidthPayloadOnly()
    {
        NetworkManager.BandwidthPayloadOnly = m_BandwidthPayloadOnly;
    }

    private void Awake()
    {
        UpdateBandwidthPayloadOnly();
    }
