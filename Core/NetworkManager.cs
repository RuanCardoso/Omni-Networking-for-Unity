...
namespace Omni.Core
{
    ...
    public partial class NetworkManager : MonoBehaviour, ITransporterReceive
    {
        ...
        /// </summary>
        public static bool IsTickTimingEnabled { get; private set; } = false;
        ...
        {
            if (__O_Keys__.__Internal__Key__ == null)
            {
                // __Internal__Key__ is generated by the source generator in runtime
                ...
                case Module.NtpClock:
                    {
                        ...
                        if (nClock != null)
                        {
                            ...
                            IsTickTimingEnabled = nClock.UseTickTiming;
                        }
        ...
        public virtual async void Internal_OnServerPeerConnected(IPEndPoint peer, NativePeer nativePeer)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    string encryptedRsaPublicKey = StringEncryptor.Encrypt(ServerSide.RsaPublicKey, GUID);
                        ...
                        case MessageType.NtpQuery:
                            {
                                if (isServer)
                                {
                                    ...
                                    Sntp.Server.SendNtpResponse(time, peer);
                                }
                                else
                                {
                        ...
                        case MessageType.BeginHandshake:
                            {
                                if (!isServer)
                                {
                                    ...
                                    rsaServerPublicKey = StringEncryptor.Decrypt(rsaServerPublicKey, GUID);
                                    if (!RsaEncryptor.Validate(tokenBytes, tokenSignature, rsaServerPublicKey))
                                ...
                                else
                                {
                                    ...
                                    byte[] encryptedServerAesKey = AesEncryptor.Encrypt(serverAesKey, 0,
                                        serverAesKey.Length,
                        ...
                        case MessageType.EndHandshake:
                            {
                                if (!isServer)
                                {
                                    ...
                                    ClientSide.ServerPeer._aesKey = AesEncryptor.Decrypt(serverAesKeyEncrypted, 0,
        ...
        void OnGUI()
        {
            ...
            if (IsClientActive)
            {
                statsContent += $"{pingColor}Rtt:</color> {LocalPeer.Ping} ms";

                if (m_SntpModule)
                {
                    statsContent += $" ({Sntp.Client.Ping} ms)\n";
                    statsContent += $"{timeColor}Time:</color> {LocalPeer.Time} sec\n";
                    statsContent += $"{timeColor}Synced Time:</color> {Sntp.Client.Time}";
                }
                else
                {
                    statsContent += $"\n{timeColor}Time:</color> {LocalPeer.Time} sec";
                }
            }
