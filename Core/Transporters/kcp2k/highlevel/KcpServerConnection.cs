...
namespace kcp2k
{
    public class KcpServerConnection : KcpPeer
    {
        ...
        protected override void OnDisconnected() => OnDisconnectedCallback();

        protected override void OnError(ErrorCode error, string message) =>
            OnErrorCallback(error, message);

        protected override void RawSend(ArraySegment<byte> data) => RawSendCallback(data);
        ...
        public void RawInput(ArraySegment<byte> segment)
        {
            ...
            switch (channel)
            {
                ...
                default:
                {
                    ...
                    Log.Warning(
                        $"[KCP] ServerConnection: invalid channel header: {channel}, likely internet noise"
                    );
                    break;
                }
            }
